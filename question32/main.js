...
var current_users = ['admin', 'John', 'Parker', 'Peter', 'Dakota'];
var new_users = ['admin', 'John', 'Sevenna', 'Johnson', 'Jacob'];
new_users.forEach(function (newUsername) {
    ...
    if (current_users.map(function (c_users) { return c_users.toLowerCase(); }).includes(lowerCase)) {
        console.log("\nThe username ".concat(newUsername, " is taken. You will need to enter a new username."));
    }
    else {
        console.log("\nThe username: ".concat(newUsername, ", is available."));
    }
});
