...
let current_users: string [] = ['admin', 'John', 'Parker', 'Peter', 'Dakota'];
let new_users: string [] = ['admin', 'John', 'Sevenna', 'Johnson', 'Jacob'];

new_users.forEach(newUsername => {
    let lowerCase = newUsername.toLowerCase();
if (current_users.map(c_users => c_users.toLowerCase()).includes(lowerCase)) {

    console.log(`\nThe username ${newUsername} is taken. You will need to enter a new username.`);
}
else {
